  ...
  return {
    ...
    methods: {
      refresh() {
        ...
        _this.loading = true;
        var interface = this.$refs['content'].value;
        var paramStr = this.$refs['exinfo'].value || "{}";
        if (!interface) {
          dsf.layer.message('请填写接口地址', false);
          return;
        }
        this.$http.get(interface, JSON.parse(paramStr))
          .done(function (res) {
            if (res.success) {
              _this.$refs.meta.clearRows();
              var dataAll = _this.getColumnKey(res.data);
