dsf.define(function () {
  return {
    ...
    dataLoaded: function (data, next) {
      if (!data['dsfa_mm.ID']) {
        data['dsfa_mm.ID'] = this.queryString.path;
      }
      next();
    },
    //页面全部加载完成，且元素都已经挂载
    ready: function () {
      this.prefix = this.$viewData['dsfa_mm.ID'];
    },
    //表单保存前钩子函数
    ...
    //=======二次开发额外增加的钩子函数 end   ===============
    methods: {
      setFullCode: function () {
        let arr = this.prefix.split(".");
        arr = _.filter(arr, (el) => {
          return el
        });
        if (this.$viewData["dsfa_mm.code"]) {
          arr.push(this.$viewData["dsfa_mm.code"])
        }

        this.$viewData["dsfa_mm.ID"] = arr.join(".")
      }
    }
  }
});
