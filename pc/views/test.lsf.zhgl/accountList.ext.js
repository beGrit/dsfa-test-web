  ...
  return {
    ...
    methods: {
      ...
      enabled(row) {
        ...
        var id = row.data['_id'];
        this.$http.post("/pc/test/lsf/zhgl/enabled?id=" + id)
          .done(res => {
            if (res.success === false) {
              throw new Error(res.message);
            } else {
              dsf.layer.message("启用成功");
              this.reloadData();
            }
          })
          .error(reason => {
      ...
      disEnabled(row) {
        ...
        var id = row.data['_id'];
        this.$http.post("/pc/test/lsf/zhgl/disEnabled?id=" + id)
          .done(res => {
            if (res.success === false) {
              throw new Error(res.message);
            } else {
              dsf.layer.message("禁用成功");
              this.reloadData();
            }
          })
          .error(reason => {
      ...
      resetPassword(row) {
        ...
        let id = row.data['_id'];
        this.$http.post("/pc/test/lsf/zhgl/resetPassword?id=" + id)
          .done(res => {
            if (res.success === false) {
              throw new Error(res.message);
            } else {
              dsf.layer.message("重置密码成功: " + res.data.newPassword);
              this.reloadData();
            }
          })
          .error(reason => {
