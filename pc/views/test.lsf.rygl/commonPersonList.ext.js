  ...
  return {
    ...
    methods: {
      ...
      commit(row) {
        ...
        var accountName = row.data['test_lsf_rygl.accountname'];
        var nickName = row.data['test_lsf_rygl.nickname'];
        if (accountName === undefined || nickName === undefined) {
          dsf.layer.message('提交审核的时候必须填写账号信息!', false);
        } else {
          var id = row.data['_id']
          this.$http.post('/pc/test/lsf/shgl/commit?id=' + id)
            .done((res) => {
              if (res.success === false) {
                throw new Error(res.message);
              } else {
                dsf.layer.message("提交成功")
                this.reloadData();
              }
            })
            .error(reason => {
